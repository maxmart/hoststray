...
app.whenReady().then(() => {
    ...
    const hosts = fs.readFileSync(path, 'utf8');

    const groups = getGroups(hosts);
    console.log({groups})

    const contextMenu = Menu.buildFromTemplate(groups.map(group => 
        ({ 
            label: group.name, 
            type: 'checkbox', 
...
function toggleGroup(group, enable) {
    const hosts = fs.readFileSync(path, 'utf8');
    const lines = new String(hosts).split("\n");
    let ingroup = false;
    for (let i=0; i<lines.length; i++) {
        let line = lines[i];
        if (line.startsWith("#====")) {
            const _group = line.substring("#====".length).trim();
            if (_group == group) {
                ingroup = true;
            } else {
                ingroup = false;
    ...
    fs.writeFileSync(path, newhosts, {encoding:"utf8"});
}

function getGroups(hosts) {
    const lines = new String(hosts).split("\n");
    const groups = [];
    let currentGroup = null;
    for (let line of lines) {
        if (line.startsWith("#====")) {
            const group = line.substring("#====".length).trim();
            if (group.length > 0) {
                currentGroup = {name: group, enabled: true};
            } else {
